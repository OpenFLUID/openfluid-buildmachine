...
def subprocessCall(Command, FilePath="", CommandCwd=".", OutputInShell=False, OutputAsReturn=False): # TODO check if "." == os.getcwd() in every case
    ...
    if OutputInShell:
        ...
        try:
            if mergeLogs:
                P = subprocess.Popen(Command,cwd=CommandCwd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
                out = P.communicate()[0]
            else:
